...
class PuntoDigitalApp {
  ...
  /**
   * Renderiza productos (primeros 4)
   */
  renderProducts() {
    ...
    if (!products || !products.length) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--text-gray)">
          <i class="fas fa-box-open" style="font-size:48px;color:rgba(212,168,67,0.3);display:block;margin-bottom:16px"></i>
          <p style="font-size:16px">Próximamente nuevos productos</p>
        </div>`;
      return;
    }
  ...
  renderFeaturedProducts() {
    ...
    if (!displayProducts.length) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--text-gray)">
          <i class="fas fa-star" style="font-size:48px;color:rgba(212,168,67,0.3);display:block;margin-bottom:16px"></i>
          <p style="font-size:16px">Próximamente productos destacados</p>
        </div>`;
      return;
    }
...
// Re-renderizar cuando Firestore cargue los datos reales
window.addEventListener('siteDataReady', () => {
  if (window.app) {
    window.app.renderInitialContent();
  }
});
